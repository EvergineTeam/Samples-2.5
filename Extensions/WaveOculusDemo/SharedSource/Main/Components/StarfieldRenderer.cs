...
namespace WaveOculusDemoProject.Components
{
    ...
    public class StarfieldRenderer : Drawable3D
    {
        ...
        private static readonly VertexPositionColorTexture[] referenceData = new VertexPositionColorTexture[]
        {
        ...
        private static readonly StarColorRamp[] colorRamp = new StarColorRamp[]
        {
        ...
        private Color ColorFromLerp(float lerp)
        {
            ...
            {
                index = i;
                if (colorRamp[i].Lerp >= lerp)
                {
                    break;
                }
            }

            lerp = (lerp - colorRamp[index - 1].Lerp) * (colorRamp[index].Lerp - colorRamp[index - 1].Lerp);
            return colorRamp[index].Color * lerp + colorRamp[index - 1].Color * (1 - lerp);
        }
    }
}
